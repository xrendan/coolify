...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/xrendan/coolify/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("coolify.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in coolify.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "coolify.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
